...
7. **Long multi-tool tasks:** stream a short plan first when useful. The harness posts live status lines (`🌤️ 查询天气` / `⚙️ 编码中` / …) from tool events — do not restate those progress lines; final channel answer is the deliverable.
