...
pub fn t<'a>(lang: Language, key: &'a str) -> &'a str {
    match key {
        ...
        // —— ZeroClaw (deep-fused second ACP backend, no Plugins card) ——
        "zeroclaw.status_unknown" => tr(lang, "检测中", "Checking…"),
        "zeroclaw.status_cloning" => tr(lang, "正在获取 ZeroClaw 源码…", "Fetching ZeroClaw source…"),
        "zeroclaw.status_building" => tr(lang, "正在构建 ZeroClaw…", "Building ZeroClaw…"),
        "zeroclaw.status_configuring" => tr(lang, "正在生成 ZeroClaw 配置…", "Generating ZeroClaw config…"),
        "zeroclaw.status_ready" => tr(lang, "ZeroClaw 就绪", "ZeroClaw ready"),
        "zeroclaw.status_error" => tr(lang, "ZeroClaw 异常", "ZeroClaw error"),
        "zeroclaw.degrade_notice" => tr(
            lang,
            "ZeroClaw 助手正在后台准备中，本次由主 Agent 处理。",
            "ZeroClaw assistant is still preparing in the background; the primary agent handled this turn instead.",
        ),
        "zeroclaw.source_tag" => tr(lang, "ZeroClaw", "ZeroClaw"),
        // —— Per-turn routing card ——
        "route.title" => tr(lang, "路由", "Route"),
        "route.step_intent" => tr(lang, "意图分类", "Intent"),
        "route.step_target" => tr(lang, "目标后端", "Target"),
        "route.step_zc_status" => tr(lang, "ZeroClaw 状态", "ZeroClaw status"),
        "route.step_dispatch" => tr(lang, "实际派发", "Dispatched to"),
        "route.intent_coding" => tr(lang, "编码/工程", "Coding"),
        "route.intent_general" => tr(lang, "通用助手", "General"),
        "route.backend_grok" => tr(lang, "Grok（主 Agent）", "Grok (primary)"),
        "route.backend_zc" => tr(lang, "ZeroClaw", "ZeroClaw"),
        "route.matched" => tr(lang, "命中关键词", "Matched keyword"),
        "route.default_coding" => tr(
            lang,
            "无明显非编码信号，默认编码路径",
            "No non-coding signal; default coding path",
        ),
        "route.forced_zc" => tr(lang, "手动 /zc 前缀强制", "Forced via /zc prefix"),
        "route.degraded" => tr(lang, "已降级", "Degraded"),
        "route.note" => tr(lang, "说明", "Note"),
        "route.degrade_zc_not_ready" => tr(
            lang,
            "ZeroClaw 尚未就绪，本次回落到主 Agent；后台正在自愈安装/构建",
            "ZeroClaw not ready; fell back to primary agent while self-heal runs",
        ),
        "route.degrade_zc_start_failed" => tr(
            lang,
            "ZeroClaw 桥接启动失败，本次回落到主 Agent",
            "ZeroClaw bridge failed to start; fell back to primary agent",
        ),
        "route.degrade_attachments" => tr(
            lang,
            "附件暂不支持 ZeroClaw，走主 Agent",
            "Attachments not supported on ZeroClaw yet; using primary agent",
        ),
