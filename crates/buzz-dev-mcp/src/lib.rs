...
impl DevMcp {
    ...
    #[tool(
        name = "route_pick",
        description = "Pick one eligible @Agent for a capability. Order: user preferred_name pin (if eligible) → memory preference_names soft rank → deterministic pubkey/name tie-break. Never invent an agent when this returns none — ask the user. Research defaults to ZeroClaw."
    )]
    async fn route_pick(
        &self,
