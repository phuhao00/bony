...
pub async fn cmd_set_add_policy(client: &BuzzClient, policy: &str) -> Result<(), CliError> {
    ...
    // Desktop discovers agents via kind:10100 (`list_relay_agents`). Include
    // directory fields so external room agents show human names + online status
    // rather than bare npub after `set-add-policy`.
    let display_name = std::env::var("BUZZ_ACP_DISPLAY_NAME")
        .ok()
        .map(|s| s.trim().to_string())
        .filter(|s| !s.is_empty());
    let respond_to = std::env::var("BUZZ_ACP_RESPOND_TO")
        .ok()
        .map(|s| s.trim().to_string())
        .filter(|s| !s.is_empty());
