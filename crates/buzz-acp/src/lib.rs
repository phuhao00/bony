...
/// Publish kind:0 metadata + kind:10100 agent directory so Desktop can list this
/// external harness (`list_relay_agents`) and show a human name.
async fn publish_directory_profile(
    ...
    let profile_builder = buzz_sdk::build_profile(Some(name), Some(name), None, None, None)
        .map_err(|e| relay::RelayError::Http(format!("profile build error: {e}")))?;
...
/// Tender Market posts must not wake every subscribe=all seat.
///
/// - `【招标任务】` — exclusive work dispatch; only p-tagged winners run.
/// - `【招标结果】` — settlement notice; no agent should start a turn.
fn should_skip_tender_market_event(event: &nostr::Event, self_pubkey_hex: &str) -> bool {
    let content = event.content.as_str();
    if content.contains("【招标结果】") {
        return true;
    }
    if content.contains("【招标任务】") {
        return !event_mentions_agent_ci(event, self_pubkey_hex);
    }
    false
}

fn event_mentions_agent_ci(event: &nostr::Event, agent_pubkey_hex: &str) -> bool {
    let want = agent_pubkey_hex.to_ascii_lowercase();
    event.tags.iter().any(|t| {
        let parts = t.as_slice();
        parts.first().map(|s| s.as_str()) == Some("p")
            && parts
                .get(1)
                .is_some_and(|pk| pk.eq_ignore_ascii_case(&want))
    })
}
...
async fn tokio_main() -> Result<()> {
    ...
    // Directory profile so external buzz-acp room agents appear in Desktop
    // agent discovery (kind:10100) and have a display name (kind:0).
    let directory_name = std::env::var("BUZZ_ACP_DISPLAY_NAME")
        .ok()
        .filter(|s| !s.trim().is_empty());
    let respond_to_label = match &config.respond_to {
        RespondTo::Anyone => "anyone",
        RespondTo::OwnerOnly => "owner_only",
        RespondTo::Allowlist => "allowlist",
        RespondTo::Nobody => "nobody",
    };
    match publish_directory_profile(
        &presence_publisher,
        &presence_keys,
    ...
    .await
    {
        Ok(_) => tracing::info!("published agent directory profile"),
        Err(e) => tracing::warn!("failed to publish agent directory profile: {e}"),
    }
                    ...
                    match buzz_event {
                        Some(buzz_event) => {
                            ...
                            // it never revokes same-owner team bots.
                            {
                                ...
                                if should_skip_tender_market_event(&buzz_event.event, &pubkey_hex)
                                {
                                    tracing::debug!(
                                        channel_id = %buzz_event.channel_id,
                                        author = %author,
                                        "tender market exclusive / notice — skipping turn"
                                    );
                                    continue;
                                }
                            }
