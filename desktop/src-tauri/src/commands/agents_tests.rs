...
fn deploy_payload_matches_the_shared_full_launch_fixture() {
    let fixture_path = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join(
        "tests/fixtures/deploy-full-launch.request.json",
    );
...
#[test]
fn tauri_platform_configs_do_not_bundle_kubernetes() {
    ...
    for target in [Target::MacOS, Target::Linux, Target::Windows] {
        ...
        assert!(
            !has_kubernetes,
            "Kubernetes externalBin should be gone for {target}; merged {paths:?}"
        );
    }
}
