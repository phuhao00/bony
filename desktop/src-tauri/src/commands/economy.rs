...
#[tauri::command]
pub async fn economy_cancel_tender(tender_id: String) -> Result<TenderSnapshot, String> {
    tokio::task::spawn_blocking(move || {
        crate::managed_agents::economy::cancel_tender(&tender_id)
    })
    .await
    .map_err(|e| format!("spawn_blocking failed: {e}"))?
}
...
#[tauri::command]
pub async fn economy_clear_tenders(mode: String) -> Result<u32, String> {
    tokio::task::spawn_blocking(move || {
        crate::managed_agents::economy::clear_tenders(&mode).map(|n| n as u32)
    })
    .await
    .map_err(|e| format!("spawn_blocking failed: {e}"))?
}
