...
fn built_in_room_capabilities_distinguish_coding_and_tool_seats() {
    ...
    let mut docsmith = minimal_record(&"bb".repeat(32));
    docsmith.name = "DocSmith".to_string();
    docsmith.start_on_app_launch = true;
    docsmith.env_vars.insert(
        crate::managed_agents::types::MANAGED_AGENT_CAPABILITIES_ENV.to_string(),
        "document.create".to_string(),
    );
    let docsmith_capabilities = super::built_in_room_capabilities(&docsmith);
    assert_eq!(docsmith_capabilities, vec!["document.create"]);
    assert!(!docsmith_capabilities
        .iter()
