...
export type TenderBid = {
  ...
  bidderKind: string;
  stake: number;
  /** Stats-based asking price */
  quote?: number;
  quoteBasis?: string | null;
  reputationAtBid?: number | null;
  note?: string | null;
};
...
export type TenderSnapshot = {
  ...
  /** excellent | pass | thin | fail — from settlement schedule */
  rewardGrade?: string | null;
  /** Human-readable schedule breakdown */
  rewardNote?: string | null;
  rewardTags?: string[];
  rewardAchievements?: string[];
  rewardCapabilities?: string[];
  /** Bid board + winner rationale */
  allocation?: AllocationDecision | null;
  bids: TenderBid[];
  createdAt: string;
};

export type TenderClearMode = "stuck" | "history" | "all";
...
export async function cancelEconomyTender(
  tenderId: string,
): Promise<TenderSnapshot> {
  return await invokeTauri<TenderSnapshot>("economy_cancel_tender", {
    tenderId,
  });
}

export async function clearEconomyTenders(
  mode: TenderClearMode,
): Promise<number> {
  return await invokeTauri<number>("economy_clear_tenders", { mode });
}
...
/**
 * Idempotently create the local room agent (ZeroClaw)
 * and their shared "Local Room" channel, natively —
