...
export function useEconomyAdminMutation() {
  ...
  const cancelTender = useMutation({
    mutationFn: (tenderId: string) => cancelEconomyTender(tenderId),
    onSettled: invalidate,
  });
  const clearTenders = useMutation({
    mutationFn: (mode: "stuck" | "history" | "all") => clearEconomyTenders(mode),
    onSettled: invalidate,
  });
  return {
    adjustBalance,
