#![deny(unsafe_code)]
#![warn(missing_docs)]
//! buzz-db — Sqlite event store for Buzz.
//!
//! ## Design invariants
...
//! - Single-instance deployment: no read replicas, no monthly table partitioning.
...
/// Push a dynamically-sized `IN (?, ?, ..., ?)` list onto `qb`.
///
/// SQLite (unlike Postgres) has no array bind type — `= ANY($1)` with a
/// slice bind is not supported (no `Encode`/`Type` impl for `[T]`/`Vec<T>`
/// on the SQLite driver). Every "match any of these ids/keys" query is
/// rewritten to build an `IN (...)` clause with one bind per element
/// instead. Callers must check for an empty `items` first — `IN ()` is
/// invalid SQL — and short-circuit to an empty result.
pub(crate) fn push_in_list<T>(
...
/// The session that served a channel-window read (see
/// [`Db::get_channel_window_with_session`]).
///
/// Single-instance SQLite has exactly one pool — there is no replica to pin
/// a snapshot against — so this no longer distinguishes a replica session
/// from the writer. It exists purely to keep the return shape (and the
/// request-scoped follow-up query call sites in `buzz-relay`) unchanged.
...
/// Snapshot of Sqlite connection pool utilisation.
...
/// Marks the usage-metrics collection poller as the (only) leader.
///
/// Postgres deployments used a session advisory lock here to elect exactly
/// one leader across multiple relay pods. Single-instance SQLite has no
/// other pod to contend with, so [`Db::try_lock_usage_metrics`] always
/// succeeds; this still holds a detached connection so the poller's
/// liveness ping (`is_live`) has something real to probe.
...
#[derive(Debug, Clone)]
pub struct DbConfig {
    /// SQLite connection URL (usually sourced from `DATABASE_URL`), e.g.
    /// `sqlite:///path/to/buzz.db` or `sqlite::memory:` for tests. The
    /// database file is created on first run — see [`Db::connect_pool`].
...
impl Default for DbConfig {
    /// Single-instance desktop/local deployment: one writer, modest pool.
    /// SQLite serializes writers at the file level regardless of pool size,
    /// so a large pool only helps concurrent readers.
    fn default() -> Self {
        Self {
            database_url: "sqlite::memory:".to_string(), // sadscan:disable np.sqlite.1
            max_connections: 10,
            min_connections: 1,
...
impl Db {
    /// Creates a new `Db` by connecting a SQLite pool with the given config.
    ///
    /// `config.database_url` is a file path (`sqlite:///path/to/buzz.db`) or
    /// `sqlite::memory:` for an ephemeral in-process database (tests). The
    /// database file is created on first run if it does not already exist.
    ...
    ///
    /// Enables `PRAGMA foreign_keys = ON` on every connection — SQLite
    /// leaves foreign-key enforcement off by default (unlike Postgres,
    /// which always enforces `REFERENCES` constraints), so this is required
    ...
    /// Creates a `Db` from an existing `SqlitePool` (useful in tests).
    ...
    /// Marks the caller as the usage-metrics collection leader.
    ///
    /// Single-instance SQLite has no other process to contend with, so this
    /// always succeeds (`lock_key` is accepted for API compatibility with
    /// the multi-pod Postgres callers but is otherwise unused). See
    /// [`UsageMetricsLeader`].
    ...
    ///
    /// Returns a `'static` transaction because `SqlitePool` is `Arc`-backed internally.
    ...
    pub async fn ensure_configured_community(
        ...
        // SQLite has no `xmax`-style system column to distinguish an insert
        // from a no-op conflict update in one RETURNING statement, so this is
        // split into an insert attempt (DO NOTHING) followed by a fallback
        // read on conflict. Both statements run on the same pool, and
        // SQLite's single-writer serialization makes the two-step sequence
        // race-free for a single-instance deployment. `id` also has no
        // DB-side default (SQLite has no server-side random-UUID generator),
        // so it is generated here; a losing insert simply discards it.
        ...
        let (id, host, created) = if let Some(row) = inserted {
            let id: Uuid = row.try_get("id")?;
            let host: String = row.try_get("host")?;
            (id, host, true)
        } else {
            let row = sqlx::query(
                r#"
                SELECT id, host FROM communities WHERE lower(host) = lower($1)
                "#,
            )
            .bind(normalized_host)
            .fetch_one(&self.pool)
    ...
    pub async fn create_community_with_owner(
        ...
        // owner cannot both pass the ownership count check. On Postgres this
        // held a per-owner advisory lock; SQLite's single-connection pool
        // already serializes every transaction at connection-checkout time
        // (see `crate::DbConfig`), so a second concurrent call blocks here at
        // `self.pool.begin()` until this transaction commits or rolls back —
        // no explicit lock is needed to get the same total ordering.
        // `communities.id` has no DB-side default (SQLite has no server-side
        // random-UUID generator): generate it in Rust.
        let new_id = Uuid::new_v4();
        let row = sqlx::query(
            r#"
            INSERT INTO communities (id, host)
    ...
    pub async fn archive_community_owned_by(
        ...
        let row = sqlx::query(
            ...
            // unqualified — no alias, no FROM-joined columns.
            r#"UPDATE communities AS c
               SET archived_at = COALESCE(c.archived_at, strftime('%Y-%m-%dT%H:%M:%fZ','now'))
    ...
    pub async fn communities_of_channels(
        ...
        let mut qb: QueryBuilder<Sqlite> = QueryBuilder::new(
            "SELECT id, community_id FROM channels WHERE id IN ",
        );
        push_in_list(&mut qb, channel_ids.iter().copied());
    ...
    /// Formerly [`Db::query_events`] with replica routing. Single-instance
    /// SQLite has only one pool, so this is now a direct alias; `path` is
    ...
    /// Formerly [`Db::query_events_routed`] restricted to the bounded arm.
    /// Now a direct alias — see [`Db::query_events_routed`].
    ...
    /// Formerly [`Db::count_events`] with replica routing. Now a direct
    /// alias — see [`Db::query_events_routed`].
    pub async fn count_events_routed(&self, _path: &'static str, q: &EventQuery) -> Result<i64> {
        event::count_events(&self.pool, q).await
    }
    ...
    /// Formerly [`Db::get_events_by_ids`] with replica routing. Now a
    /// direct alias — see [`Db::query_events_routed`].
    ...
    /// the same connection. Single-instance SQLite has only one pool, so
    /// the session is always a thin wrapper over that pool.
    ...
    /// Formerly [`Db::query_feed_mentions`] with replica routing. Now a
    /// direct alias — see [`Db::query_events_routed`].
    ...
    /// Formerly [`Db::query_feed_needs_action`] with replica routing. Now a
    /// direct alias — see [`Db::query_feed_mentions_routed`].
    ...
    /// Formerly [`Db::query_feed_activity`] with replica routing. Now a
    /// direct alias — see [`Db::query_feed_mentions_routed`].
    ...
    pub async fn touch_api_token(&self, community_id: CommunityId, hash: &[u8]) -> Result<()> {
        sqlx::query(
            "UPDATE api_tokens SET last_used_at = strftime('%Y-%m-%dT%H:%M:%fZ','now') WHERE community_id = $1 AND token_hash = $2",
        )
        .bind(community_id.as_uuid())
        .bind(hash)
        .execute(&self.pool)
    ...
    pub async fn replace_addressable_event(
        ...
        // Single-instance SQLite: no advisory locks exist, but the whole
        // check-then-write below runs inside one transaction, and SQLite's
        // single-writer model serializes concurrent transactions for us —
        // no separate lock acquisition is needed.
        let mut tx = self.pool.begin().await?;
    ...
    pub async fn publish_nip43_membership_locked(
        ...
        // Single-instance SQLite has no advisory locks; the read-build-write
        // cycle below runs inside one transaction, and SQLite's
        // single-writer model serializes concurrent transactions for us.
        let mut tx = self.pool.begin().await?;

        // Read current members inside the transaction.
        let rows = sqlx::query(
    ...
    pub async fn replace_parameterized_event(
        ...
        // Single-instance SQLite: no advisory locks; the check-then-write
        // below runs inside one transaction, serialized by SQLite itself.
        let mut tx = self.pool.begin().await?;
        ...
        if existing.is_some() {
            // Always soft-delete here, even for the NIP-RS/buzz-mesh-status
            // kinds that end up physically purged (`hard_delete_superseded`).
            // The Postgres schema instead issued a direct hard `DELETE`
            // gated behind a transaction-local `set_config`/`current_setting`
            // GUC, so a stale pre-fix writer binary (running concurrently
            // during a rolling multi-pod upgrade) could not bypass the
            // corrected path. SQLite has no session GUCs and, more
            // importantly, no such rolling-upgrade scenario exists for a
            // single-instance deployment with exactly one writer binary:
            // `events_purge_soft_deleted_nip_rs`/`..._mesh_status` in the
            // migration convert this soft-delete into the same physical
            // purge (event row + its mentions) unconditionally at the
            // trigger level, with no caller-identity gate needed.
            sqlx::query(
                "UPDATE events SET deleted_at = strftime('%Y-%m-%dT%H:%M:%fZ','now') \
                 WHERE community_id = $1 AND kind = $2 AND pubkey = $3 AND d_tag = $4 AND deleted_at IS NULL",
            )
...
fn parse_api_token_row(row: sqlx::sqlite::SqliteRow) -> Result<ApiTokenRecord> {
    let id: Uuid = row.try_get("id")?;
...
mod tests {
    ...
    async fn nip_rs_replacement_hard_deletes_payload_and_watermark_rejects_replay() {
        ...
        sqlx::query(
            "UPDATE events SET deleted_at=strftime('%Y-%m-%dT%H:%M:%fZ','now') WHERE community_id=$1 AND kind=30078 AND pubkey=$2 AND d_tag=$3",
        )
        .bind(community.as_uuid())
        .bind(keys.public_key().to_bytes().as_slice())
        .bind(&d_tag)
    ...
    /// The `events_reject_live_*_hard_delete` triggers reject a direct hard
    /// `DELETE` of a still-live row at a purge-eligible coordinate, whether
    /// or not the deleting statement's own event happens to be a
    /// Rust-classified `is_nip_rs`/`is_buzz_mesh_status` one — the guard
    /// matches on the stored row's own kind/d_tag/tags, not the caller's
    /// classification of some other event. A row at an unrelated kind is
    /// unaffected. (Postgres additionally gated the equivalent guard behind
    /// a transaction-local `set_config`/`current_setting` GUC to protect
    /// against a stale writer binary during a rolling multi-pod upgrade;
    /// SQLite has no session GUCs and no such scenario for a single-writer
    /// deployment, so there is nothing analogous left to test here.)
    #[tokio::test]
    async fn nip_rs_hard_delete_fence_fails_closed_for_live_rows() {
        use nostr::{EventBuilder, Keys, Kind, Tag, Timestamp};
        ...
        sqlx::query(
            "INSERT INTO event_mentions \
             ...
             VALUES ($1, $2, $3, strftime('%Y-%m-%dT%H:%M:%fZ', $4, 'unixepoch'), 30078)",
        )
        .bind(community.as_uuid())
        .bind("6".repeat(64))
        ...
        // Match on `(community_id, id)` alone rather than also filtering by
        // `created_at`: the event id is a content hash that already commits
        // to a unique `created_at`, and comparing `created_at` against a
        // `strftime(...)`-formatted literal is fragile — sqlx's chrono
        // encoding for `TEXT` columns does not necessarily produce a
        // byte-identical string to `strftime('%Y-%m-%dT%H:%M:%fZ', ...)`, so
        // an exact-string equality filter can silently match zero rows
        // instead of erroring.
        let rejected_nonconforming = sqlx::query("DELETE FROM events WHERE community_id=$1 AND id=$2")
            .bind(community.as_uuid())
        ...
        let unrelated_delete = sqlx::query("DELETE FROM events WHERE community_id=$1 AND id=$2")
            .bind(community.as_uuid())
    ...
    async fn database_guard_covers_legacy_writer_and_nip09_deletion() {
        ...
        async fn legacy_insert(
            ...
            sqlx::query(
                "INSERT INTO events (community_id, id, pubkey, created_at, kind, tags, content, sig, received_at, d_tag) \
                 VALUES ($1, $2, $3, $4, $5, $6, $7, $8, strftime('%Y-%m-%dT%H:%M:%fZ','now'), $9) ON CONFLICT DO NOTHING",
            )
            .bind(community.as_uuid())
            .bind(event.id.as_bytes().as_slice())
            .bind(event.pubkey.to_bytes().as_slice())
            // Bind the same `chrono::DateTime<Utc>` type production code uses
            // for this column (see `replace_parameterized_event`) rather than
            // a `strftime(..., 'unixepoch')`-formatted string: sqlx's chrono
            // encoding for SQLite emits RFC3339 with a `+00:00` offset and no
            // fractional digits when nanos are zero, which is not byte-equal
            // to `strftime`'s `.000Z`-suffixed output — mixing the two would
            // make this row's primary key silently fail to match rows
            // written through the normal chrono-bound path.
            .bind(
        ...
        sqlx::query(
            "INSERT INTO event_mentions \
                 (community_id, pubkey_hex, event_id, event_created_at, event_kind) \
             VALUES ($1, $2, $3, strftime('%Y-%m-%dT%H:%M:%fZ', $4, 'unixepoch'), 30078) ON CONFLICT DO NOTHING",
        )
        .bind(community.as_uuid())
        .bind("e".repeat(64))
        .bind(b.id.as_bytes().as_slice())
        ...
        // An exact replay of the still-live C row must be a zero-row
        // trigger/`ON CONFLICT DO NOTHING` no-op regardless of any other
        // in-flight writer. Postgres could exercise this concurrently with
        // the delete below (row-level KEY SHARE locking lets independent
        // statements interleave); SQLite's single-connection test pool
        // serializes every statement at connection checkout (see
        // `setup_db`), so there is no third connection available to run
        // this replay while the delete task below is queued for the only
        // one. Check the no-op invariant sequentially instead — the
        // invariant under test (an exact replay never resurrects C) does
        // not depend on the deletion being concurrently in-flight.
        let exact_replay = legacy_insert(&db.pool, community, &c, &d_tag)
            .await
            .expect("exact C replay is skipped");
        assert_eq!(exact_replay.rows_affected(), 0);

        // Exercise legacy UPDATE-trigger deletion with the same barrier used
        // for B above: hold C's live-event lock in one transaction while a
        // concurrent delete waits for the connection, then release it and
        // confirm the delete completes and purges C.
    ...
    #[tokio::test]
    async fn test_usage_metrics_lock_always_succeeds_on_single_instance() {
        // Single-instance SQLite has no advisory locks and no multi-pod
        // leader election: every caller of `try_lock_usage_metrics` becomes
        // a "leader" immediately (see the method's doc comment), unlike the
        // old multi-pod Postgres contract this test used to pin.
        let db = setup_db().await;
        ...
        let mut first = db
            ...
            .expect("single-instance lock always succeeds");
        assert!(first.is_live().await, "lock owner remains reachable");

        assert!(
            db.try_lock_usage_metrics(key)
                .await
                .expect("second lock attempt")
                .is_some(),
            "single-instance SQLite has no leader election — every caller succeeds"
        );
    ...
    async fn create_community_with_owner_enforces_per_owner_limit() {
        ...
        // Create up to the max number of communities for this owner.
        ...
        let host = format!(
            "limit-test-{}-{}.example",
            relay_members::MAX_COMMUNITIES_PER_OWNER,
            Uuid::new_v4().simple()
        );
        assert_eq!(
            db.create_community_with_owner(&host, &owner)
    ...
    async fn insert_channel(pool: &SqlitePool, community_id: Uuid, channel_id: Uuid) {
