...
const SUCCESS_REP_MATCH: i32 = 10;
const SUCCESS_REP_MISMATCH: i32 = 25;
const FAIL_REP_MATCH: i32 = -5;
const FAIL_REP_MISMATCH: i32 = -30;
const FAIL_REP_BROKER: i32 = -8;
const FAIL_PENALTY_FRACTION_BPS: u32 = 2500;
...
pub fn settle(paths: &EconomyPaths, p: SettleParams) -> Result<SettleResult, EconomyError> {
    ...
    if success {
        let rep = if executor.mismatch {
            SUCCESS_REP_MISMATCH
        } else {
            SUCCESS_REP_MATCH
        };
        append_chained(
            ...
            &LedgerEntry {
                ...
                amount: executor.budget,
                reputation_delta: rep,
                task_ref: Some(executor.task_ref.clone()),
                note: note.clone().or_else(|| {
                    Some(format!(
                        "payout for {} (mismatch={})",
                        executor.contract_id, executor.mismatch
                    ))
                }),
    ...
    let new_capabilities = if success {
...
pub fn pick_auction_winner(
    ...
    let mut scored: Vec<AuctionCandidate> = agents
        ...
        .map(|a| {
            ...
            let match_ratio = capability_match_ratio(&a.capabilities, capability);
